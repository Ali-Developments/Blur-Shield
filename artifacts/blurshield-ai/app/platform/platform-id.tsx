...
import { submitYoutubeJob, pollYoutubeJob, youtubeResultUrl } from '@/lib/youtubeMusicApi';
...
export default function PlatformDetailScreen() {
  ...
  const redirectUri = isOAuthCapable ? getRedirectUri(platformId as OAuthPlatformId) : '';
  const discovery   = isOAuthCapable ? getDiscovery(platformId as OAuthPlatformId) : null;
  ...
  const [request, response, promptAsync] = AuthSession.useAuthRequest(
    ...
    discovery ?? { authorizationEndpoint: 'https://example.com/authorize' },
  );

  // Google OAuth hook — used exclusively for the YouTube platform.
  // expo-auth-session/providers/google handles platform-native redirect URIs
  // and the authorization-code → token exchange automatically.
  ...
  // Provide non-undefined fallbacks so expo-auth-session doesn't throw an
  // invariant error on web when these env vars aren't configured yet.
